...
@Inject internal constructor(
    ...
    private val _fetchCurrentWeatherResult = MutableLiveData<Result<Unit>>()
    val fetchCurrentWeatherResult: LiveData<Result<Unit>> = _fetchCurrentWeatherResult

    val weatherHistory = liveData(Dispatchers.IO) {
        weatherRepository.getWeatherHistory().collect { emit(it) }
    }

    val currentWeather = weatherHistory.map { it.firstOrNull() }
    ...
    fun fetchCurrentWeather(lat: Double, lon: Double) {
        viewModelScope.launch {
            ...
            try {
                ...
                _fetchCurrentWeatherResult.value = Result.Success(Unit)
            } catch (e: Exception) {
